...
if($Password!=""){
 ...
 $query="update `users` set `UserID`='$UserID',`Salt`='$Salt',`Password`='$Password',`Email`='$Email',`FirstName`='$FirstName',`LastName`='$LastName',`Address`='$Address',`ZipCode`='$ZipCode',`State`='$State',`Phone`='$Phone' where `ID`='$ID'";

		if($conn->query($query)===true){
			$msg="<div class='alert alert-success' color='green'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a><strong>Record has been updated</strong></div>";
	}else{
	$errmsg="<div class='alert alert-warning' color='red'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a><strong>Record has been not updat</strong></div>";
	}
...
<a class="btn btn-link pull-right" href="index.php"><span class="glyphicon glyphicon-arrow-left"></span> Back</a></h2>   
</div>
    </div>
 </div>
